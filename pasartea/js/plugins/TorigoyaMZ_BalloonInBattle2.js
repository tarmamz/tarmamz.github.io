 ...
 * @author Ruたん（ru_shalm）- Translation by Tarma
 ...
 * https://torigoya-plugin.rutan.dev
 *
 * Displays dialogue in speech ballons above the character during battle。
 *
 * ------------------------------------------------------------
 * ■ How to use
 * ------------------------------------------------------------
 *
 * (1) Register a dialogue set
 *
 * You must register sets from the plugin settings menu.
 * Select "Register Set" and put as many as you need.
 *
 * In order to use the line set IDs you set in the plugin,
 * it is better to use names easy to remember (for example: Alex)
 *
 * (2) Reflecting dialouges on actors and enemies
 *
 * In the note tags of an actor or an enemy. Set
 * the ID of the line set you want to use for an action.
 *
 * <Set: Alex>
 *
 * With this, you're good to go and start seeing pop ups during battle.
 *
 * ------------------------------------------------------------
 * ■ FAQ
 * ------------------------------------------------------------
 ...
 * 
 * Q. Why this translation sounds like broken english?
 * 
 * Due my lack of knowledge on japanese, I used DeepL to translate it roughly so I could use the plugin a bit easier.
 * English is not my language mother either so yeah, sorry for the typos and wrong phrases.
 *
 * ------------------------------------------------------------
 * ■ Advance use
 * ------------------------------------------------------------
 *
 * ● Secret tags than can be used in dialogues
 *
 * There are several secret tags.
 * However, some of them may not work in some situations。
 *
 * \target
 * 　The name of the person who will receive the skill or item.
 * 　In the case of a skill or item that affects several targets,
 *   the name of the first target will be used.
 *
 * \from
 * 　The name of the person who used the skill or item.
 * 　相手が特定できない場合は空欄になります
 * 　※バトルイベントによるものなど
 *
 * ----------
 *
 * ● Set a condition for each line.
 *
 * 各セリフのメモ欄に <条件: ～～> の形式で
 * 少し特別な条件を記述することができます。
